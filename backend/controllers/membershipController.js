...
const buyMembership = async (req, res) => {
  try {
    const userId = req.user.id;
    const { type, price, razorpay_payment_id } = req.body;
    ...
    if (!referralCodeForUser) {
      ...
      user = await User.findByIdAndUpdate(
        userId,
        { customID: referralCodeForUser, referralLink },
        { new: true }
      );
    } else {
      referralLink = user.referralLink || `${BASE_URL}/register?ref=${referralCodeForUser}`;
      if (!user.referralLink) {
    ...
    // ✅ Update Referral to mark membership bought
    await Referral.findOneAndUpdate(
      { referredUser: userId },
      { hasBoughtMembership: true }
    );
...
const getMyReferrals = async (req, res) => {
  try {
    ...
    const onlyRegistered = referrals
      .filter((r) => !r.hasBoughtMembership && r.referredUser)
